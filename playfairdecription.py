...
a = b = 0
#Getting user inputs Key (to make the 5x5 char matrix) and Cipher Text (Message that is to be decripted)
key = input("Enter key: ")
key = key.replace(" ", "")
key = key.upper()
    ...
    for i,j in enumerate(ciphermatrix):
        for k,l in enumerate(j):
            if x == l:
                ...
                lst.append(k)
                return lst
#decription takes in the ciphertext and decripts it using the row, rectangle, or column method
#from the playfair cipher decription method
def decription(text):
    i=0
